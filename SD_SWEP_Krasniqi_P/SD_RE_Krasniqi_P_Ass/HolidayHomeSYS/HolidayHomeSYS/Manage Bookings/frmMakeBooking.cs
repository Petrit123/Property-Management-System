...
namespace HolidayHomeSYS
{
    public partial class frmMakeBooking : Form
    {
        ...
        private void txtSname_TextChanged(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Error! This textbox only accepts letters only", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                txtSname.Text = "";
                txtSname.Focus();
                return;
            }
        ...
        private void confirmbtn_Click(object sender, EventArgs e)
        {
            ...
            booking.setTenant_Id(Convert.ToInt32(txtTenantID.Text));
            tenant.setTenantID(Convert.ToInt32(txtTenantID.Text));
            booking.setBooking_Id(Convert.ToInt32(txtBooking.Text));
            booking.setProp_No(Convert.ToInt16(grdProperties.SelectedCells[0].Value));
            booking.setBooking_Status("B");
            booking.setDateFrom(arrivalDate.Value);
            booking.setDateTo(DepartureDate.Value);
            booking.setAmount((Convert.ToDateTime(DepartureDate.Text) - Convert.ToDateTime(arrivalDate.Text)).TotalDays * Property.getPropRate(pNo));
            tenant.setSurname(txtSname.Text);
            tenant.setForename(txtFname.Text);
            tenant.setPhoneNumber(Convert.ToInt32(txtPhone.Text));
            tenant.setCardNumber(Convert.ToInt32(txtCard.Text));

           

            tenant.regTenant();
            booking.insertBooking();
            txtTenantID.Text = Tenants.nextTenant_Id().ToString("0000");
            txtBooking.Text = Bookings.nextBookingID().ToString("0000");
        ...
        private void Searchbtn_Click(object sender, EventArgs e)
        {
            ...
            if(arrivalDate.Text  == DepartureDate.Text)
            {
                MessageBox.Show("Error! Departure date cannot be the same as arrival date!", "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
                DepartureDate.Focus();
                return;
            }
